...
pv.check = function(pv,bCheckEmpty = FALSE,bCheckSort = TRUE,bDoVectors =
                       TRUE) {
   if (missing(pv)) {
...
pv.version = function(pv,v1,v2,v3) {
   ...
   if (warn) {
      warning('Loading DBA object from a previous version -- updating...',call. =
                 F)
   }
   
   if (!is.null(pv$contrasts)) {
      for (i in 1:length(pv$contrasts)) {
         if (!is.null(pv$contrasts[[i]]$DESeq) &
             is.null(pv$contrasts[[i]]$DESeq1) &
...
pv.setScore = function(pv,score,bLog = F,minMaxval,rescore = TRUE,filterFun =
                          max,bSignal2Noise = T) {
   doscore = TRUE
   ...
   if (doscore) {
      if ((score >= DBA_SCORE_TMM_MINUS_FULL) &&
         ...
         if (score == DBA_SCORE_TMM_MINUS_EFFECTIVE ||
             score == DBA_SCORE_TMM_MINUS_EFFECTIVE_CPM) {
            bMinus   = TRUE
            bFullLib = FALSE
         ...
         pv$allvectors[,4:ncol(pv$allvectors)] = pv.normTMM(pv,bMinus =
                                                               bMinus,bFullLib = bFullLib,bCPM = bCPM)
         ...
         for (i in 1:length(pv$peaks)) {
            ...
            if (score == PV_SCORE_RPKM) {
               ...
               pv$allvectors[,colnum] = pv$peaks[[i]]$Reads - pv$peaks[[i]]$cReads
            } else if (score == PV_SCORE_SUMMIT ||
                       score == PV_SCORE_SUMMIT_ADJ) {
               ...
               if (is.null(pv$peaks[[i]]$Summits)) {
                  warning(
                     'DBA_SCORE_SUMMIT_POS not available; re-run dba.count with summits=0'
                  )
               } else {
   ...
   if (!missing(minMaxval)) {
      ...
      if (sum(tokeep) > 1 && (sum(tokeep) < length(tokeep))) {
         ...
         #                 annotation = sort(pv$annotation)[tokeep]
         #             }
         for (i in 1:length(pv$peaks)) {
         ...
         pv = pv.vectors(
            pv,minOverlap = 1,bAnalysis = F,bAllSame = T
         )
         if (rescore) {
            pv$maxFilter = 0
            pv$score = NULL
            pv = pv.setScore(
               pv,score = score,bLog = bLog,minMaxval = 0,bSignal2Noise = F
            )
         }
...
pv.contrast3 = function(vectors,n1,n2,n3,minVal = 0,v1,v2,v3) {
   ...
   res = list(
      onlyA = vectors[onlyA,c(1:3,(n1 + 3))],
      onlyB = vectors[onlyB,c(1:3,(n2 + 3))],
...
pv.contrast4 = function(vectors,n1,n2,n3,n4,minVal = 0,v1,v2,v3,v4) {
   ...
   res = list(
      ...
      inAll = vectors[inAll,c(1:3,(n1 + 3),(n2 + 3),(n3 + 3),(n4 +
                                                                 3))]
   )
...
pv.analysis = function(pv,attributes = pv$attributes,bPCA = T,distMeth =
                          "pearson") {
   #require(amap)
   
   peaks  = pv$vectors
   values = matrix(as.numeric(as.matrix(peaks[,4:ncol(peaks)])),nrow(peaks),ncol(peaks) -
                      3)
...
pv.do_peaks2bed = function(peaks,chrmap = NULL,fn,numCols = 4) {
   ...
   if (!is.null(fn)) {
      ...
      options(scipen = 8)
      write.table(
         peaks[,1:mcols],file = fn,#sprintf("%s.bed",fn),
...
pv.namestrings = function(crec1,crec2,crec3,crec4) {
   ...
   for (i in 1:length(crec1)) {
      if ((crec1[i] == crec2[i]) &&
          (crec1[i] == crec3[i]) && (crec1[i] == crec4[i])) {
   ...
   return(list(
      n1 = s1,n2 = s2,n3 = s3,n4 = s4,tstring = t1
   ))
}
...
pv.dovectors = function(allpeaks,classes,bKeepAll = F,maxgap = 0,useExternal =
                           TRUE,useC = TRUE) {
   if (!useC) {
...
pv.CalledMasks = function(pv,newpv,master) {
   ...
   spare = pv.peakset(
      pv,master,peak.caller = 'raw',scoreCol = 4,bLowerScoreBetter = F
   )
...
pv.matchingSamples = function(id,classes) {
   res = which(!classes[PV_CALLER,] %in% "counts" &
                  classes[PV_ID,] %in% id)
   return(res)
}
...
pv.pairs = function(pv,mask,bPlot = F,attributes = pv$attributes,bAllVecs =
                       T,
                    CorMethod = "pearson",bCorOnly = F,bNonZeroCors = F,minVal =
                       0,bFixConstantVecs = T) {
   if (missing(mask)) {
   ...
   for (first in 1:(numSets - 1)) {
      ...
      for (second in (first + 1):numSets) {
         ...
         if (sd(v1) && sd(v2)) {
            ...
            corval = 0
         resm = rbind(resm,c(
            which(mask)[first],which(mask)[second],
...
pv.colsv = c(
   crukBlue, crukMagenta, crukCyan,crukGrey,
...
pv.pcmask = function(pv,numSites, mask, sites,removeComps,cor = F,bLog =
                        T) {
   if (missing(numSites))
...
pv.venn2 = function(mrec,n1,n2,...) {
   ...
   vennDiagram(
      res,names = c(n1,n2), circle.col = 2:3,counts.col = 1,...
   )
}
...
pv.venn2 = function(olaps,l1,l2,main = "",sub = "") {
   ...
   counts = list(counts)
   vennPlot(
      counts,setlabels = c(l1,l2),mysub = sub,mymain = main
   )
...
pv.venn3 = function(m3way,n1,n2,n3,...) {
   ...
   vennDiagram(
      res,names = c(n1,n2,n3), circle.col = 2:4,counts.col = 1,...
   )
}
...
pv.venn3 = function(olaps,l1,l2,l3,main = "",sub = "") {
   ...
   counts = list(counts)
   vennPlot(
      counts,setlabels = c(l1,l2,l3),mysub = sub,mymain = main
   )
...
pv.venn4 = function(olaps,l1,l2,l3,l4,main = "",sub = "") {
   ...
   names(counts) = c(
      "A","B","C","D","A_B","A_C","A_D","B_C","B_D","C_D",
      "A_B_C","A_B_D","A_C_D","B_C_D","A_B_C_D"
   )
   counts = list(counts)
   
   vennPlot(
      counts,setlabels = c(l1,l2,l3,l4),mysub = sub,mymain = main
   )
...
pv.peaksetCounts = function(pv = NULL,peaks,counts,
                            sampID = "",tissue = "",factor = "",condition =
                               "",treatment = "",replicate) {
   if (!is.null(pv$peaks)) {
   ...
   froms = NULL
   tos   = NULL
   if (is.null(dim(counts)) && length(counts) == 1) {
      # filename
   ...
   peaks = cbind(
      peaks,counts,counts,rep(0,numcounts),counts,rep(0,numcounts),rep(0,numcounts)
   )
